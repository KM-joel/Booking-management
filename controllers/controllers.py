...
class PortalReservation(CustomerPortal):
    ...
    def create_reservation_records(self, **post):
        try:
            values = {
                'client_id': r.env.uid,
                'article_id': r.env['product.product'].sudo().search([('id', '=', post.get('type_article'))], limit=1).id,
            ...
            recs = r.env['booking.management.reservation'].sudo().create(values)
            return self.portal_my_reservations()
        except Exception as e:
            return self.view_reservation_form_create(error=e)

    @http.route(['/my/reservation/<int:reservation_id>', '/my/reservation/<int:reservation_id>/<access_token>'], type='http',
                auth='public', website=True)
    def open_view_detail_reservation(self, reservation_id=None, access_token=None, **kw):
        try:
            reservation_sudo = self._document_check_access('booking.management.reservation', reservation_id, access_token)
