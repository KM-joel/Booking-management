...
class SaleOrder(models.Model):
    ...
    def _compute_total_reservation(self):
        for rec in self:
            rec.total_reservation = len(rec.reservation_ids)
    ...
    def open_view_reservations(self):
        ...
        # ._for_xml_id('booking_management.reservation_all_action_window')
    ...
    confirmed_user_id = fields.Many2one(
        related="reservation_ids.client_id", string="Confirmed user"
    )
    ...
    count_client_group = fields.Integer(
        string="Count in group client", compute="_compute_count_client_group"
    )

    @api.constrains("reservation_ids")
    def _compute_count_client_group(self):
        group_by_client = self.env["booking.management.reservation"].read_group(
            domain=[], fields=["client_id"], groupby=["client_id"]
        )
        for rec in group_by_client:
            client_id = rec.get("client_id")[0]
            client_rec = self.browse(client_id)
            client_rec.count_client_group = rec["client_id_count"]
            self -= client_rec
    ...
    # 	logger.info('Action in animation when confirmed reservation & effect arc_en_ciel')
    # 	return {
    # 		'effect': {
    # 			'fadeout': 'slow',
