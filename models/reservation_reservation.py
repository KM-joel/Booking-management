...
class Reservation(models.Model):
    ...
    end_date_reservation = fields.Date('End date of reservation', readonly=True, tracking=True,
    ...
    reference_record = fields.Reference(selection=[('sale.order', 'Sales'), ('account.move', 'Invoices')],
                                        string='Records')
    ...
    def name_get(self):
        return [(record.id, '[%s]/%s/%s' % (record.reference, record.client_id.name, record.article_id.name)) for record
                in self]
    ...
    @api.depends('reservation_date', 'reservation_duration_hours', 'reservation_duration_day',
    ...
    def next_step(self):
        ...
        if self.state == 'new':
            ...
            type_notif = 'success' if self.env['booking.management.reservation'].search_count(
                [('client_id', '=', self.client_id.id), ('state', '=', 'validated')]) > 1 else 'warning'
    ...
    def create_quote(self):
        self.ensure_one()
        duration = (
                           self.reservation_duration_month * 30 + self.reservation_duration_day) * 24 + self.reservation_duration_hours
        price = 0
